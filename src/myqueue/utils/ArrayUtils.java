...
public class ArrayUtils {
    ...
    */
     ...
     */
    public static int[][] insertRowInto2DArray(int[][] matrix, int row, int[] data) {
        int lenght;
        if (matrix != null) {
            if (matrix[0] == null) {
                return new int[][]{data};
                ...
                int[][] out = new int[matrix.length + 1][];
                for (int i = 0; i < row; i++) {
                    out[i] = matrix[i];
                }
                out[row] = data;
                for (int i = row + 1; i < out.length; i++) {
                    out[i] = matrix[i - 1];
                }
                return out;
            }
    ...
    /**
     * Удаление строки с массива
     */
    public static int[][] removeRowFrom2dArray(int[][] array, int row){
        int rows = array.length;
        int[][] arrayToReturn = new int[rows-1][];
        if (row >= 0) System.arraycopy(array, 0, arrayToReturn, 0, row);
